...
## 输出要求
1. 默认直接回答用户问题，不要把普通问题扩写成完整报告
2. 涉及最新数据、行情或近期事件时先联网搜索；证据不足必须标注“待核验”
3. 先给结论，再给最多 3 条关键依据和最多 3 条风险/待核验事项
4. 只有关键数字或对比关系需要表格；聊天回答最多 1 张表
5. 用户明确要求“生成报告/导出/下载”时，再使用正式报告结构
6. 以"---\\n*以上分析仅供参考，不构成投资建议。*"结尾"""
