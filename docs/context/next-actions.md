...
建立最小固定评测集并完成真实模型质量评估：记录结构化输出、事实与数字、引用覆盖、融资需求和事件影响质量；不并行启动第二个 agent。

## Why This Is Next

企业尽调对象、证据、融资需求、行动建议和事件影响历史回放已经落地；本地确定性 stub 与 1 次真实 DeepSeek 回放已验证“材料 → 资产 → 事件影响 → 持久化 → 人工确认 → 审计记录”和权限隔离。Claude Code Round 1 已合并，21 个后端测试通过，前端构建通过，Provider 错误分类和排序稳定性已修复。当前还缺少多样本固定评测和人工质量记录。
