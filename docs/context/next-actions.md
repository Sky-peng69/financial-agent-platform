...
补齐公开 Demo 部署入口：确定后端公开 URL、前端托管授权和回滚方案；具备条件后只执行一次生产部署与线上回归；不并行启动第二个 agent。

## Why This Is Next

企业尽调对象、证据、融资需求、行动建议和事件影响历史回放已经落地；本地确定性 stub、1 次真实 DeepSeek 单样本回放、4 个行业合成样本回放和浏览器视觉验收已验证“材料 → 资产 → 事件影响 → 持久化 → 人工确认 → 审计记录”和权限隔离。固定评测、初步人工抽查、前端构建、后端测试和密钥扫描均已完成；当前只缺公开后端 URL 与托管授权，真实企业质量仍不能由合成材料证明。
