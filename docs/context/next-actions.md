...
在获得有效 DeepSeek 凭证后，执行一次真实模型端到端回放并记录结构化输出、引用选择和事件影响质量；凭证恢复前不修改外部密钥、不把真实模型质量标记为已验证。不并行启动第二个 agent。

## Why This Is Next

企业尽调对象、证据、融资需求、行动建议和事件影响历史回放已经落地；本地确定性 stub 已验证“事件 → 影响判断 → 持久化历史 → 新行动建议 → 人工确认 → 审计记录”和权限隔离。Claude Code Round 1 已合并，21 个后端测试通过，前端构建通过，Provider 错误分类和排序稳定性已修复。真实 DeepSeek 回放在生成资产时返回 401，待凭证恢复后再单独验证。
