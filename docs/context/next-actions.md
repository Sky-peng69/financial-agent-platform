...
补齐 `FinancingNeed` 最小持久化闭环；不并行启动第二个 agent，也不同时推进事件影响持久化。

## Why This Is Next

企业尽调对象、证据、行动建议和事件影响预览已经落地；本地确定性 stub 已验证“事件 → 影响判断 → 新行动建议 → 人工确认 → 审计记录”和权限隔离。Claude Code Round 1 已合并，21 个后端测试通过，前端构建通过，Provider 错误分类和排序稳定性已修复。当前仍缺少融资需求持久化；真实 DeepSeek 回放在生成资产时返回 401，待凭证恢复后再单独验证。
