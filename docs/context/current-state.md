...
企业金融尽调 MVP 的设计和实施计划已确认；确定性 stub 和 1 次真实 DeepSeek 端到端回放均已完成，Claude Code Round 1 后端测试已合并，最终整合验收通过。融资需求和事件影响结果均已完成最小持久化、历史查询和人工复核边界。当前唯一下一项是建立固定评测集并进行人工质量评估。现有 `ResearchSubject` 作为兼容持久化对象保留，前端语义逐步切换为企业金融对象。
